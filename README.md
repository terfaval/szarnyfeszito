...
- Responses are validated against the v2.1 dossier schema (`schema_version`, `header`, `pill_meta`, `short_options`, `long_paragraphs`, `identification`, `distribution`, `nesting`, `migration`, `fun_fact`, `ethics_tip`, `typical_places`). Validation failures return `422` along with `issues` and the model name so the admin can triage missing fields.
