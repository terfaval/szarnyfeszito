...
type PhenomenonEditorHeaderProps = {
  phenomenon: { id: string; title: string; slug: string; status: PhenomenonStatus };
  links: {
...
export function PhenomenonEditorHeader({ phenomenon, links }: PhenomenonEditorHeaderProps) {
    ...
    <Card className="place-panel stack">
      <header className="place-panel-header admin-header-row">
        <div className="admin-heading">
          <p className="admin-heading__label">Phenomenon editor</p>
          <h1 className="admin-heading__title admin-heading__title--large">{phenomenon.title}</h1>
          <p className="admin-heading__description">Slug: {phenomenon.slug}</p>
        </div>

        <div className="flex flex-wrap items-center gap-3">
          <StatusPill status={phenomenon.status} />
          <Link className="btn btn--ghost" href="/admin/phenomena">
            Back to list
          </Link>
        </div>
      </header>

      <nav className="admin-tablist" aria-label="Phenomenon editor sections">
        {items.map((item) => {
...
export default PhenomenonEditorHeader;
